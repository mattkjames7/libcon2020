...
int main() {
	...
	GetCon2020Params(&mui,&irho,&r0,&r1,&d,&xt,&xp,eqtype,&Edwards,&ErrChk,
					...
					&wO_open,&wO_oc,&thetamm,&dthetamm,&thetaoc,&dthetaoc);
...
| `azfunc` | `"connerney"` | `SetAzimuthalFunc()` <br /> `GetAzimuthalFunc()` | Set the method to use to calculate the azimuthal field due to the radial component of the current: <br /> `"connerney"` - use the Connerney et al., 2020 azimuthal field model <br /> `"lmic"` - use the L-MIC model (Connerney et al., 2005, 2007, 2008, 2017). |
| `wO_open` | `0.1` | `SetOmegaOpen()` <br /> `GetOmegaOpen()` | Ratio of plasma to Jupiter's angular velocity on open field lines. |
| `wO_oc` | `0.35` | `SetOmegaOC()` <br /> `GetOmegaOC()` | Ratio of plasma to Jupiter's angular velocity in the outer magnetosphere. |
